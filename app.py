...
# ------------------------------------------------
# 🛠️ 숨은 기본 여백을 완전히 지워버리는 강력한 CSS
# ------------------------------------------------
st.markdown("""
    <style>
    /* 모바일 브라우저의 강제 화면 확대/축소 뷰포트 고정 */
    html, body, [data-testid="stAppViewContainer"] {
        max-width: 100vw !important;
        overflow-x: hidden !important;
    }
    
    /* 컬럼 블록 간의 기본 좌우 간격(gap) 제거 */
    ...
    /* 컬럼 내부의 숨은 여백과 최소 너비 제한 완전히 해제 */
    div[data-testid="column"] {
        min-width: 0px !important;
        padding: 0px 1px !important;
    }
    
    /* 텍스트가 절대 다음 줄로 밀리지 않도록 고정 */
    div[data-testid="stMarkdownContainer"] p {
        white-space: nowrap !important;
        word-break: keep-all !important;
    }

    /* 📱 모바일 환경 전용: 글자 크기와 버튼 부피를 극단적으로 다이어트 */
    @media (max-width: 600px) {
        p, div, span {
            font-size: 10px !important;
        }
        /* 버튼 내부 여백을 0으로 만들어 부피 축소 */
        .stButton > button {
            width: 100% !important;
    ...
    @media (min-width: 601px) {
        ...
        .stButton > button { font-size: 13px !important; padding: 4px !important; }
    }
    </style>
...
if is_open:
    ...
    st.error("🚨 **경기가 완전히 종료되었습니다!** 최종 당환 결과를 확인하세요.")
...
# ------------------------------------------------
# 📊 현재 생존 현황 (여백 제로 기반 순수 7열 콤팩트 그리드)
# ------------------------------------------------
...
if not df.empty:
    ...
    # 가로 비율을 7글자 이내 단어 규격에 맞춰 조밀하게 정렬
    grid_cols = st.columns([1.6, 1.1, 1.1, 1.1, 1.3, 0.9, 0.9])
    ...
    for index, row in df.iterrows():
        r_cols = st.columns([1.6, 1.1, 1.1, 1.1, 1.3, 0.9, 0.9])
